...
public class MyTokenizer extends StreamTokenizer {
	 ...
	 * @param r
	 */
	public MyTokenizer(Reader r) {
		super(r);
		commentChar('\'');
		slashSlashComments(false);
		slashStarComments(false);
		lowerCaseMode(true);
		//I think this is everything but the "
		ordinaryChar('/');
		ordinaryChar('-');
		ordinaryChar(' ');
		ordinaryChar('\t');
	...
	/**
	 * peekNext is identical to nextToken(), except that you may
	 * call putBack() to put the old tokens back. Similar to pushBack(), 
	...
	public void parseWord() throws IOException{
		...
		while(Character.isLetterOrDigit((char)super.nextToken())){
			str.append((char)ttype);
		}
		ttype = TT_WORD;
		sval = str.toString();
	...
	public void parseNumber() throws IOException{
		int val = ttype - '0';
		while(Character.isDigit((char)super.nextToken())){
			val *=10;
			val +=ttype - '0';
		}
		//now we are checking for a decimal or a range
		if (ttype == '.'){
			//if there is nothing significant about this decimal, we want to roll back after
			//so we save our values
			peek_nval = nval;
			peek_sval = sval;
			peek_type = ttype;
			peek_oval = oval;
		...
		/*	}else */if (Character.isDigit((char)super.nextToken()/*ttype*/)){
				/*
				 * We have a float value
				 */
				
				float fval = val;
				int decval = 10;			//start at first digit to the right of the decimal
