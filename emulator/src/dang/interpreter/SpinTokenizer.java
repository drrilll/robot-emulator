...
public class SpinTokenizer {
	 ...
	 * @param r
	 */
	public SpinTokenizer(String stream) {
	...
	/*	case '\t':
		...
		case '{':
			...
			if (ttype == '{'){
				//we have '{{' comments
				while (true){
					int toke = superNextToken();
					while ((toke != '}')&&(toke!=TT_EOF)&&(toke!=TT_EOL)){
						if (position > size -1){
							throw new SyntaxError("Line: "+lineno()+" unmatched {{");
						}
						toke= superNextToken();
					}
					if ((toke == TT_EOF)||(toke== TT_EOL)) return nextToken();
					if (superNextToken() == '}')return nextToken();
					else throw new SyntaxError("expected second }");
				}
				//return nextToken();
				...
				int toke = superNextToken();
				//Debug.debug("toke: "+(char)toke,"toke");
				while ((toke != '}')&&(toke!=TT_EOF)&&(toke!=TT_EOL)){
					if (position > size -1){
						throw new SyntaxError("Line: "+lineno()+" unmatched {");
					}
		...
		case '\'':
			...
			while((superNextToken()!=TT_EOL)&&(superNextToken()!=TT_EOF)){}
			pushBack();
			break;
		/**************************************************/
		/* Math operators */
