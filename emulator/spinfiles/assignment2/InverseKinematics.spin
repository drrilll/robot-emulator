...
'how I do it here, but I may change my approach in the future
  ...
  RBC.DebugStrCr(FS.FloatToString(F.FSQR(4.0)))
  RBC.DebugStrCr(string("atan values"))

  'the atan2 is weird. it takes the y coordinate first, then
  'the x. took a long time to figure that one out.

  RBC.DebugStrCr(FS.FloatToString(F.Degrees(F.ATAN2(1.0,0.0))))
  RBC.DebugStrCr(FS.FloatToString(F.Degrees(F.ATAN2(1.0,-1.0))))
  RBC.DebugStrCr(FS.FloatToString(F.Degrees(F.ATAN2(-1.0,0.0))))
  RBC.DebugStrCr(FS.FloatToString(F.Degrees(F.ATAN2(0.0,1.0))))
...
PRI Pivot (deltaAngle) | pivotCount , xangle
  ResetCount
  if (F.FROUND(deltaAngle)>180)
    xangle := F.FABS(F.FSUB(deltaAngle, 360.0))
    spin := RIGHT
  else
    xangle := deltaAngle
    spin := LEFT
